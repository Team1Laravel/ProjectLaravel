                    ...
                    success: function (res) {
                        ...
                        for (const val of res.moviegenres) { 
                        	htmlmg += '<div id="inputFormRow_u">';
                       		...
                       		htmlmg += '<select name="genre_id_u[]" id="genre_id_u" class="form-control" required="required">';
                       		htmlmg += '<option value="">Select genre</option>'
                       		for (const tmp of allGenres){
                       			if(val.id === tmp.id){
                       				htmlmg += '<option value="'+tmp.id+'" selected>'+tmp.name+'</option>'
                       			}else{
                       				htmlmg += '<option value="'+tmp.id+'">'+tmp.name+'</option>'
                       			}
                       		}
                            htmlmg += '</select>';
                            htmlmg += '<div class="input-group-append"><button id="removeRow_u" type="button" class="btn btn-danger">Remove</button></div></div></div>';
                        }                             
